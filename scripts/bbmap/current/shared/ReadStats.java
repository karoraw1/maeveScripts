...
//		}
	...
	public void writeGCToFile(String fname, boolean printZeros){
		...
		TextStreamWriter tsw=new TextStreamWriter(fname, overwrite, false, false);
		tsw.start();
		tsw.print("#Mean\t"+String.format(Locale.ROOT, "%.3f", Tools.averageHistogram(hist)*gcMult)+"\n");
		tsw.print("#Median\t"+String.format(Locale.ROOT, "%.3f", Tools.percentileHistogram(hist, 0.5)*gcMult)+"\n");
		tsw.print("#Mode\t"+String.format(Locale.ROOT, "%.3f", Tools.calcModeHistogram(hist)*gcMult)+"\n");
		tsw.print("#STDev\t"+String.format(Locale.ROOT, "%.3f", Tools.standardDeviationHistogram(hist)*gcMult)+"\n");
		if(GC_PLOT_X){
