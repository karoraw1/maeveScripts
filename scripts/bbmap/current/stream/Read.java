...
//		}
		...
		if(SKIP_SLOW_VALIDATION){
			...
			if(U_TO_T){uToT();}
//			passesJunk=validateCommonCase_branchless(processAssertions); //For some reason this is slower than branchy mode
				...
				if(JUNK_MODE==FIX_JUNK){
					...
					if(processAssertions){
						KillSwitch.kill("\nAn input file appears to be misformatted:\n"
							+ "The character with ASCII code "+bases[1]+" appeared where "+(aa ? "an amino acid" : "a base")+" was expected.\n"
							+ "Sequence #"+numericID+"\n"
							+ "Sequence ID '"+id+"'\n"
	...
	private boolean validateCommonCase_branchless(boolean processAssertions){
		...
		if(foundJunk){
			if(JUNK_MODE==FIX_JUNK){
				...
				if(processAssertions){
					KillSwitch.kill("\nAn input file appears to be misformatted:\n"
							+ "The character with ASCII code "+bases[1]+" appeared wherea base was expected.\n"
							+ "Sequence #"+numericID+"\n"
							+ "Sequence ID '"+id+"'\n"
							+ "Sequence: "+Tools.toStringSafe(bases)+"\n\n"
							+ "This can be bypassed with the flag 'tossjunk', 'fixjunk', or 'ignorejunk'");
...
//		System.out.println(Arrays.toString(matchToMsdicn(match)));
	...
	public int countSubs(){
		assert(match!=null && valid() && !shortmatch()) : (match!=null)+", "+valid()+", "+shortmatch();
...
//		assert(i==0) : i+"\n"+this+"\n"+new String(match)+"\n"+Arrays.toString(new int[] {m, s, d, i, n, splice});
	...
	public int pairLength(){return length()+mateCount();}
	public int pairCount(){return 1+mateLength();}
