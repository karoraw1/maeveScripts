...
//		System.err.println(params[2]+", "+ancestor);
		...
		if(names.length==1){
			return toJson(type, names[0], simple, !simple, printNumChildren, printChildren, printPath, printSize, printRange, source).toString();
		}
		ArrayList<JsonObject> list=new ArrayList<JsonObject>();
		for(String name : names){
			list.add(toJson(type, name, simple, !simple, printNumChildren, printChildren, printPath, printSize, printRange, source));
		}
...
//		if(printChildren){j.add(getChildren(id, originalLevel, printRange));}
		while(tn!=null && tn.levelExtended!=TaxTree.LIFE_E && tn.id!=TaxTree.CELLULAR_ORGANISMS_ID){
			if(!skipNonCanonical || tn.isSimple()){
				j.add(toJson(tn, originalLevel, printNumChildren, printChildren, printPath, printSize, printRange, source, -1));
			}
	...
	JsonObject makeChildrenObject(ArrayList<TaxNode> list, boolean originalLevel, boolean printRange){
		...
		JsonObject j=new JsonObject("children");
		for(TaxNode tn : list){
			JsonObject child=new JsonObject(tn.id+"");
...
//		if(verbose2){outstream.println("In toSemicolon; type="+type+", type2="+type2+", made "+sb);}
	...
	JsonObject toJson(final int type, final String name, boolean skipNonCanonical, boolean originalLevel, 
		...
		if(type==GI){
			...
			return new JsonObject(""+type,"error","Bad type; should be gi, taxid, or name; e.g. /name/homo_sapiens");
		}
		tn=tn0;
		...
		if(tn!=null){
			...
			if(printChildren && (tn.id==tn.pid || tn.id==131567)){j.add(getChildren(tn.id, originalLevel, printRange));}
			while(tn!=null && tn.levelExtended!=TaxTree.LIFE_E && tn.id!=TaxTree.CELLULAR_ORGANISMS_ID){
//				System.err.println(tn+", "+(!skipNonCanonical)+", "+tn.isSimple());
				if(!skipNonCanonical || tn.isSimple()){
					j.add(toJson(tn, originalLevel, printNumChildren, printChildren, printPath && tn==tn0, printSize, printRange, source, img));
//					System.err.println(j);
	...
	JsonObject toJson(TaxNode tn, boolean originalLevel, boolean printNumChildren, 
		...
		if(printChildren){j.add(getChildren(tn.id, originalLevel, printRange));}
		return j;
	}
