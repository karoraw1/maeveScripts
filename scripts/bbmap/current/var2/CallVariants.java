...
//		}
	...
	public VarMap process(Timer t){
		...
		if(prefilter){
			...
			kca=prefilter(varFilter.minReads);
...
//		assert(false) : filter.toString(properPairRate, ploidy);
		...
		if(ffout!=null || vcf!=null){
			...
			if(ffout!=null){
				varMap.writeVarFile(ffout, varFilter);
			}
			if(vcf!=null){
				varMap.writeVcfFile(vcf, varFilter, sampleName, readsProcessed-readsDiscarded,
