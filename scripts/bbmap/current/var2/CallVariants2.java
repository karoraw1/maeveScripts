...
//		SamLine.PARSE_OPTIONAL=false;
		...
		for(int i=0; i<args.length; i++){
			...
			}else if(varVilter.parse(a, b, arg)){
				//do nothing
	...
	public void process(Timer t){
		...
		for(Sample sample : samples){
			...
			if(sample.vcfName!=null){
				sample.varMap.writeVcfFile(sample.vcfName, varVilter, sample.name, sample.readsProcessed-sample.readsDiscarded,
	...
	class Sample{
		...
		public long process1(VarMap sharedVarMap) {
			...
			if(prefilter){
				...
				kca=prefilter(varVilter.minReads);
...
//			assert(false) : filter.toString(properPairRate, ploidy);
		...
		private long[] processVariants(){
			return varMap.processVariantsMT(varVilter, scoreArray, ploidyArray);
		}
		
		private long[] addSharedVariants(VarMap sharedVarMap){
			return varMap.addSharedVariantsST(varVilter, sharedVarMap);
		}
...
//				System.err.println();
	...
	public final VarFilter varVilter=new VarFilter();
