...
//		bb.append(prevBase<0 ? 'N' : (char)prevBase).append('\t');
	...
	public ByteBuilder toVCF(ByteBuilder bb, float properPairRate, float totalQualityAvg, float mapqAvg, float readLengthAvg,
		...
		if(UPPER_CASE_ALLELES){prevBase=(byte) Character.toUpperCase(prevBase);}
		...
		if(addPrevBase){
			...
			if(reflen==0){
				...
				for(int i=0, rpos=start; i<reflen; i++, rpos++){
					...
					if(UPPER_CASE_ALLELES){refBase=Character.toUpperCase(refBase);}
					bb.append(refBase);
				}
			}
			bb.append('\t');
		...
		final float strandBias=strandBiasScore(scafEndDist);
		{
...
//			final int covMinus=(Scaffold.trackStrand ? scaf.minusCoverage(this) : coverage/2);
			...
			bb.append("AF=").append(String.format(Locale.ROOT, "%.4f",af)).append(';');
			bb.append("RAF=").append(String.format(Locale.ROOT, "%.4f", raf)).append(';');
			bb.append("SB=").append(String.format(Locale.ROOT, "%.4f", strandBias)).append(';');
		...
		}
		{
			...
			bb.append(String.format(Locale.ROOT, "%.4f",raf));
			bb.append(':');
			bb.append(String.format(Locale.ROOT, "%.4f",strandBias));
			bb.append(':');
...
//		final int count=count();
	...
	public float score(float properPairRate, float totalQualityAvg, float totalMapqAvg, float readLengthAvg, float rarity, int ploidy, ScafMap map){
		...
		float qs=qualityScore(totalQualityAvg, totalMapqAvg);
	...
	public float qualityScore(float totalBaseqAvg, float totalMapqAvg){
		return baseQualityScore(totalBaseqAvg)*mapQualityScore(totalMapqAvg);
	}
	
//	public float baseQualityScore(float totalBaseqAvg){
//		float bqAvg=baseQAvg();
//		final float delta=Tools.max(0, totalBaseqAvg-bqAvg);
//	}
...
//	public float mapQualityScore(float totalMapqAvg){
//		float mqAvg=mapQAvg();
//		float mqAvg2=(0.25f*(3*mqAvg+mapQMax));
//		final float delta=totalMapqAvg-mqAvg;
//		float score=(float)(1-Math.pow(10, 0-.1*(mqAvg2+4)));
//		if(delta>0){
//
//		}else{
//
